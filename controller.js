...
function _alexac($scope, $http, $q, AutoSleepService, Focus, $translate) {
	...
	function formattedName(devname, actionString) {
		...
		if (devname != undefined) {
			...
			if (s.length > 1) s.splice(1, 0, devname);
			else s.unshift(devname);
			result = s.join(" ");
		}
		return result;
	}

	function customDevices(customD) {
		//  creates your custom devices from config
		if (customD.devices) {
			for (i = 0; i < Object.keys(customD.devices).length; i++) {
				if (customD.devices[i].port === undefined) {
					customD.devices[i].port = port + 30 + i;
				}
				customD.devices[i].handler = new Function(
					"action",
					customD.devices[i].handler
				);
				customD.devices[i].context = {
					Focus: Focus,
					AutoSleepService: AutoSleepService,
				};
			}
		}
		return customD;
	...
	function menuDevices(menuD) {
		//  create your devices to control the Mirror and pi
		var opts = { timeout: 8000 };
		console.log("menu device ");
		counter = 0; // + Object.keys(menuD.devices).length
		if (menuD.devices == undefined) menuD.devices = [];
		if (config.alexaControl.refresh | true) {
			device = {};
			device.name = formattedName(
				config.alexaControl.deviceName,
				$translate.instant("alexaControl.REFRESH")
			);
			device.port = port;
			device.handler = function (action) {
				sendSocketNotification("ACTION", "refresh");
			};
		...
		if (config.alexaControl.restart | true) {
			// only with PM2
			device = {};
			device.name = formattedName(
				config.alexaControl.deviceName,
				$translate.instant("alexaControl.RESTART")
			);
			device.port = port;
			device.handler = function (action) {
				pm2.connect((err) => {
					...
					console.log(
						"Restarting PM2 process: " +
							config.alexaControl.pm2ProcessName
					);
					pm2.restart(config.alexaControl.pm2ProcessName, function (
						err,
						apps
					) {
						pm2.disconnect();
		...
		if (config.alexaControl.stop | true) {
			// only with PM2
			device = {};
			device.name = formattedName(
				config.alexaControl.deviceName,
				$translate.instant("alexaControl.STOP")
			);
			device.port = port;
			device.handler = function (action) {
				pm2.connect((err) => {
					...
					pm2.stop(config.alexaControl.pm2ProcessName, function (
						err,
						apps
					) {
						pm2.disconnect();
		...
		if (config.alexaControl.reboot | true) {
			//reboot the pi
			device = {};
			device.name = formattedName(
				config.alexaControl.deviceName,
				$translate.instant("alexaControl.REBOOT")
			);
			device.port = port;
			device.handler = function (action) {
		...
		if (config.alexaControl.shutdown | true) {
			// shutdwon the pi
			device = {};
			device.name = formattedName(
				config.alexaControl.deviceName,
				$translate.instant("alexaControl.SHUTDOWN")
			);
			device.port = port;
			device.handler = function (action) {
		...
		if (config.alexaControl.monitorToggle | true) {
			...
			device.name = formattedName(
				config.alexaControl.deviceName,
				$translate.instant("alexaControl.MONITOR")
			);
			device.port = port;
			if (config.alexaControl.blankingStyle == "vgencmd") {
				...
				device.handler = function (action) {
					console.log(
						"received monitor toggle with hide action=" + action
					);
					if (action == 0) AutoSleepService.sleep();
					else AutoSleepService.wake();
				};
			}
			menuD.devices.push(device);
			counter++;
...
angular.module("SmartMirror").controller("alexaControl", _alexac);
